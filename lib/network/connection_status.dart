...
abstract class ConnectionStatus {
  ...
  Stream<InternetConnectionEnum> get internetStream;
...
class InternetConnectionStatus implements ConnectionStatus {
  ...
  late StreamController<InternetConnectionEnum> _internetStreamController;

  InternetConnectionStatus({required Connectivity checker})
      : _checker = checker {
    _internetStreamController =
        StreamController<InternetConnectionEnum>.broadcast();
  ...
  Future<bool> get isConnected async {
    final connectivityResult = await _checker.checkConnectivity();
  ...
  void _updateConnectionStatus(List<ConnectivityResult> connectivityResult) {
    if (_isConnected(connectivityResult)) {
      _internetStreamController.add(InternetConnectionEnum.connected);
    } else {
      _internetStreamController.add(InternetConnectionEnum.disconnected);
    }
  }

  @override
  Stream<InternetConnectionEnum> get internetStream =>
      _internetStreamController.stream;
}

enum InternetConnectionEnum { waiting, disconnected, connected, none }
